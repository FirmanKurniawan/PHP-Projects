...
echo "[<a href=\"redirect-jangan.php\">All users</a>] ";
if ($dump == 0)
	echo "[<a href=\"redirect-jangan.php?id=$id&dump=1\">Include debug dump</a>] ";
else
	echo "[<a href=\"redirect-jangan.php?id=$id\">Without debug dump</a>] ";
echo "<br>\n";
...
if (strlen($row['devinfo']) > 0) {
	echo "[<a href=\"redirect-jangan.php?cmd=mo&id=$id&mo=devinfo\">DevInfo</a>]\n";
}
if (strlen($row['devdetail']) > 0) {
	echo "[<a href=\"redirect-jangan.php?cmd=mo&id=$id&mo=devdetail\">DevDetail</a>]\n";
}
if (strlen($row['pps']) > 0) {
	echo "[<a href=\"redirect-jangan.php?cmd=mo&id=$id&mo=pps\">PPS</a>]\n";
}
if (strlen($row['cert_pem']) > 0) {
	echo "[<a href=\"redirect-jangan.php?cmd=cert&id=$id\">Certificate</a>]\n";
}
echo "<BR>\n";
...
if ($row['fetch_pps'] == "1") {
	echo "On next connection " .
		"[<a href=\"redirect-jangan.php?cmd=fetch-pps-off&id=$id\">" .
		"do not fetch</a>]<br>\n";
} else {
	echo "Do not fetch " .
		"[<a href=\"redirect-jangan.php?cmd=fetch-pps-on&id=$id\">" .
		"request fetch</a>]<br>\n";
}
...
if ($rem == "") {
	echo "Not required";
	echo " [<a href=\"redirect-jangan.php?cmd=subrem-add-user&id=" .
		   $row['rowid'] . "\">add:user</a>]";
	echo " [<a href=\"redirect-jangan.php?cmd=subrem-add-machine&id=" .
		   $row['rowid'] . "\">add:machine</a>]";
	if ($row['methods'] == 'TLS') {
		echo " [<a href=\"redirect-jangan.php?cmd=subrem-add-reenroll&id=" .
			   $row['rowid'] . "\">add:reenroll</a>]";
	}
	echo " [<a href=\"redirect-jangan.php?cmd=subrem-add-policy&id=" .
		   $row['rowid'] . "\">add:policy</a>]";
	echo " [<a href=\"redirect-jangan.php?cmd=subrem-add-free&id=" .
		   $row['rowid'] . "\">add:free</a>]";
} else if ($rem == "user") {
	echo "User [<a href=\"redirect-jangan.php?cmd=subrem-clear&id=" .
		       $row['rowid'] . "\">clear</a>]";
} else if ($rem == "policy") {
	echo "Policy [<a href=\"redirect-jangan.php?cmd=subrem-clear&id=" .
		       $row['rowid'] . "\">clear</a>]";
} else if ($rem == "free") {
	echo "Free [<a href=\"redirect-jangan.php?cmd=subrem-clear&id=" .
		       $row['rowid'] . "\">clear</a>]";
} else if ($rem == "reenroll") {
	echo "Reenroll [<a href=\"redirect-jangan.php?cmd=subrem-clear&id=" .
		       $row['rowid'] . "\">clear</a>]";
} else  {
	echo "Machine [<a href=\"redirect-jangan.php?cmd=subrem-clear&id=" .
			  $row['rowid'] . "\">clear</a>]";
}
echo "<br>\n";
...
echo "<form>Policy: <select name=\"policy\" " .
	"onChange=\"window.location='redirect-jangan.php?cmd=policy&id=" .
...
echo "<form>Account type: <select name=\"type\" " .
	"onChange=\"window.location='redirect-jangan.php?cmd=account-type&id=" .
...
echo "<br>\n";
echo "<a href=\"redirect-jangan.php?cmd=reset-pw&id=" .
	 $row['rowid'] . "\">Reset AAA password</a><br>\n";

echo "<br>\n";
echo "<form action=\"redirect-jangan.php?cmd=set-osu-cred&id=" . $row['rowid'] .
  "\" method=\"POST\">\n";
...
if (strlen($row['t_c_timestamp']) > 0) {
	echo "<br>\n";
	echo "<a href=\"redirect-jangan.php?cmd=clear-t-c&id=" .
		$row['rowid'] .
...
foreach ($res as $row) {
	...
	$d = $row['dump'];
	if (strlen($d) > 0) {
		echo "[<a href=\"redirect-jangan.php?cmd=eventlog&id=" . $row['rowid'] .
		  "\">";
		if ($d[0] == '<')
		  echo "XML";
		else
...
echo "[<a href=\"redirect-jangan.php?cmd=eventlog&limit=50\">Eventlog</a>] ";
echo "<br>\n";
...
foreach ($res as $row) {
	echo "<tr><td><a href=\"redirect-jangan.php?id=" . $row['rowid'] . "\"> " .
	    $row['identity'] . " </a>";
	echo "<td>" . $row['realm'];
	$rem = $row['remediation'];
	echo "<td>";
