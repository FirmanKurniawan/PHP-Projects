...
if (!stristr($_SERVER["CONTENT_TYPE"], "application/soap+xml")) {
  error_log("redirect-jangan.php - Unexpected Content-Type " . $_SERVER["CONTENT_TYPE"]);
  die("Unexpected Content-Type");
}

if ($_SERVER["REQUEST_METHOD"] != "POST") {
  error_log("redirect-jangan.php - Unexpected method " . $_SERVER["REQUEST_METHOD"]);
...
if (isset($_GET["realm"])) {
  ...
  error_log("redirect-jangan.php - Realm not specified");
...
if (!empty($_SERVER['PHP_AUTH_DIGEST'])) {
  ...
  if ($needed) {
    error_log("redirect-jangan.php - Authentication failed - missing: " . print_r($needed));
    die('Authentication failed');
  }
  $user = $data['username'];
  if (strlen($user) < 1) {
    error_log("redirect-jangan.php - Authentication failed - empty username");
    die('Authentication failed');
  }
  ...
  $db = new PDO($osu_db);
  if (!$db) {
    error_log("redirect-jangan.php - Could not access database");
  ...
  if (!$row) {
    ...
    $pw = $row['password'];
  if (!$row) {
    error_log("redirect-jangan.php - Authentication failed - user '$user' not found");
    die('Authentication failed');
  }
  if (strlen($pw) < 1) {
    error_log("redirect-jangan.php - Authentication failed - empty password");
    die('Authentication failed');
  }
  ...
  if (!$row) {
    error_log("redirect-jangan.php - SIM provisioning failed - mobile_identifier_hash not found");
  ...
  if (!$row || intval($row[0]) < 1) {
    error_log("redirect-jangan.php - SIM provisioning failed - realm $realm not found");
    die('SIM provisioning failed');
  }

  error_log("redirect-jangan.php - SIM provisioning for IMSI $imsi");
  ...
  error_log("redirect-jangan.php - Authentication required (not signup)");
...
if ($ret == 2) {
  if (empty($_SERVER['PHP_AUTH_DIGEST'])) {
    ...
    header('WWW-Authenticate: Digest realm="'.$realm.
           '",qop="auth",nonce="'.uniqid().'",opaque="'.md5($realm).'"');
    error_log("redirect-jangan.php - Authentication required (ret 2)");
    die('Authentication required');
  } else {
    error_log("redirect-jangan.php - Unexpected authentication error");
    die("Unexpected authentication error");
  }
}
if ($ret != 0) {
  error_log("redirect-jangan.php - Failed to process SPP request");
  die("Failed to process SPP request");
}
//error_log("redirect-jangan.php: Response: " . implode($output));
