...
<head>
    <title>Cross Site Scripting | Level 3</title>
    <meta charset="utf-8">
...
?>
    ...
    <div class="container atas">
        <div class="row justify-content-center">
            <div class="col-md-5 xss">
                <h2 class="text-center">XSS Level 3</h2>
                <p>Try to alert using <b>document.URL</b></p>
                <button class="btn btn-primary mb-3" onclick="func()">Clue</button>
                <p id="clue" style="display:none">Add ' to escape the payload from value of an HTML tag</p>
                <form class="form-inline my-2 my-lg-0" action="?query=" method="get">
                    <input class="form-control mr-sm-2" type="text" placeholder="<?php echo $param; ?>" name="query" id="query">
                    <button class="btn btn-primary my-2 my-sm-0" type="submit" onclick="check()">Search</button>
                    <br>
                </form>
                <b><a href="xss4.php">Next Level</a></b>
                <b><a href="xss2.php" class="previous">Previous Level</a></b>
            </div>
            ...
            <div class="col-md-5">
                ...
                <table class="table table-borderless table-light info text-center">
                        ...
                        <?php
                            ...
                            $result = mysqli_query($db,"SELECT username FROM solved WHERE solved_level = 'LEVEL3'");
                            $jumlah = mysqli_num_rows($result);
                            $total = ceil($jumlah/$batas);

                            $data_user = mysqli_query($db, "SELECT username FROM solved WHERE solved_level = 'LEVEL3' LIMIT $halaman_awal, $batas");
                            $no = $halaman_awal+1;
    ...
    <script>
        ...
        function check() {
            ...
            if (y.value === "'<script>alert(document.URL)<\/script>") {
                alert("The flag is : KOKHEKERBANG");
            } 
        }

        function flag() {
            var z = document.getElementById("cekflag");
            if (z.value != "KOKHEKERBANG") {
                alert("Wrong flag!");
            }
        }
    </script>
